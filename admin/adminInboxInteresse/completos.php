...
<form name="s_emailAdmin" action="painel.php?exe=adminInbox/search" enctype="multipart/form-data" method="post" class=>
    ...
    <?php 					
		...
		$sql_inboxAdmin = 'select * from portal_interesses 
							...
							order by interesse_clienteData asc limit '.$inicio. ','.$maximo;
		
		 
							
							try{
								$query_inboxAdmin = $conecta->prepare($sql_inboxAdmin);
								$query_inboxAdmin->bindValue(':interesse_clienteStatus',$emailStatus,PDO::PARAM_STR);
						...
						?>
	                      ...
	                      <td align="center"><a href="painel.php?exe=adminInboxInteresse/ver&emailId=<?php echo $emailId ;?>" class=		
...
$sql_res = mysql_query("select * from portal_interesses where interesse_clienteStatus = 'completo' ORDER BY interesse_clienteData ASC");
